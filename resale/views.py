...
def checkout(request):
    if request.method=="POST":
        ...
        return render(request, 'resale/succes.html', {'thank':thank, 'id': id})
...
def msg(val,phone,id):
    ...
    message = client.messages.create( 
                                from_='whatsapp:',  
                                body='Your order ID is : ' + str(id) + '. Total amount is : ' + str(val) + '. Please pay to the given UPI ID : thrift@okhdfcbank. Please send the screenshot to thrift@gmail.com if you want your order to be shipped.'  ,
                                to='whatsapp:+916265250557' 
                            ) 
